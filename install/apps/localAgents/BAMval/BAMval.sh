...
if [ "0" -eq "1" ]; then
	...
	$samagents view '$BamFn' | cut -f3 | sort -u 
...
if [ ${#Subs} -gt 0  ]; then
	...
	cmd="$samagents view -h $BamFn  | $Subs | $samagents view -uhS - | $samagents sort - -o $bamTmp"
	echo "> $cmd"
	eval $cmd
        echo $!
...
## Sort
	...
	echo "> $samagents sort $BamFn -o $bamTmp"
	$samagents sort $BamFn -o $bamTmp
    echo $!
    if [ ! -f  $bamTmp ]; then
		echo "Error sorting $BamFn, aborting"
		exit 2
...
if [ ${#Subs} -gt 0 ] || [ $Sort -ne "0" ] || [ $Index -ne "0" ] ; then
	...
	echo "> $samagents index $BamFn"
	$samagents index $BamFn
	echo '# End time:' $(date)
	if [ ! -f  $bai ]; then
