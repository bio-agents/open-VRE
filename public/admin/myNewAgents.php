...
switch($_SESSION['User']['Type']) {
	...
	case 0:
	    $result = $GLOBALS['agentsDevMetaCol']->find();
		break;
	default:
	    $result = $GLOBALS['agentsDevMetaCol']->find(array("user_id" => $_SESSION['User']['id']));
...
<!-- BEGIN CONTENT -->
                    ...
                    <div class="page-content">
                        ...
                        <div class="page-bar">
                            <ul class="page-breadcrumb">
                              ...
                              <li>
                                  <span>Agents under development</span>
                              </li>
                            </ul>
                        </div>
                        <!-- END PAGE BAR -->
                        <!-- BEGIN PAGE TITLE-->
                        <h1 class="page-title"> Agents under development
                            <small></small>
                        </h1>
                        <!-- END PAGE TITLE-->
				<!-- END PAGE HEADER-->
			...
			<?php  
			    ...
			    <?php } ?>
					...
					<a href="admin/newAgent.php" class="btn btn-lg green" style="margin-bottom:30px;"> <i class="fa fa-plus"></i> Create new</a>

                                        <input type="hidden" id="base-url" value="<?php echo $GLOBALS['BASEURL']; ?>" />

                                        <table class="table table-striped table-bordered" id="my-new-agents" style="margin-top:30px;">
                                            <thead>
					    <tr>
						<th rowspan="2" style="vertical-align:middle">Agent (Identifier) 
                                                    <a href="javascript:;" target="_blank" class="agenttips" data-toggle="agenttip" data-trigger="hover" data-placement="top" title="Name (and identifier) for the agent being developed"><i class="icon-question"></i></a>
						</th>
                                                <th colspan="2">Wrap your code
                                                    <a href="javascript:;" target="_blank" class="agenttips" data-toggle="agenttip" data-trigger="hover" data-placement="top" title="Click the icon to open 'Bring your agent - Step 1' in a new tab"><i class="icon-question"></i></a>
                                                </th>
						<th rowspan="2" style="vertical-align:middle">Define Agent
						    <a href="javascript:;" target="_blank" class="agenttips" data-toggle="agenttip" data-trigger="hover" data-placement="top" title="Click the icon to open 'Bring your agent - Step 3' tab"><i class="icon-question"></i></a>
						</th>
                                                <th rowspan="2" style="vertical-align:middle">Submit Agent
                                                    <a href="javascript:;" target="_blank" class="agenttips" data-toggle="agenttip" data-trigger="hover" data-placement="top" title="Development status. Learn all available status by clicking the icon"><i class="icon-question"></i></a>
						</th>
						<th rowspan="2">Last update</th>
					    </tr>
                                            	<th>Generate test files
						    <a href="javascript:;" target="_blank" class="agenttips" data-toggle="agenttip" data-trigger="hover" data-placement="top" title="Click the icon to open 'Bring your agent - Step 2' in a new tab"><i class="icon-question"></i></a>
                                                </th>
				...
				<tbody>
			<?php foreach($agentsList as $v){ ?>

			<tr>
                <!-- show agent identifier -->
                <td style="padding-top:30px;">
										<?php if($_SESSION['User']['Type'] == 0) $uid = " <br> ".$v["user_id"]; ?>
										<b><?php echo $v["step3"]["agent_spec"]["name"]."&nbsp;&nbsp; ( ".$v["_id"]." )".$uid;?></b>
                    <br/><br/>
										<a href="javascript:removeAgent('<?php echo $v["_id"];?>');" class="btn btn-icon-only red agenttips"
										data-toggle="agenttip" data-trigger="hover" data-placement="bottom" title="Click here to remove this agent permanently"
										...
										<br>
										<a href="applib/createLogo.php?agentid=<?php echo $v["_id"]; ?>" class="btn green agenttips" style="margin-top:10px;"
										data-toggle="agenttip" data-trigger="hover" data-placement="bottom" title="Re-generates automatically a logo with the name of your agent."
										><i class="fa fa-file-image-o" aria-hidden="true"></i> Automatic logo</a>
										<form action="applib/uploadLogoAgent.php" method="post" class="" id="uplogo_<?php echo $v["_id"]; ?>"  enctype="multipart/form-data">
											<div class="form-group" style="margin-top:10px;">
												<input type="file" name="file" id="file_logo_<?php echo $v["_id"]; ?>" class="inputfile" />
												<label for="file_logo_<?php echo $v["_id"]; ?>" class="btn green agenttips"
												data-toggle="agenttip" data-trigger="hover" data-placement="bottom" title="You can update your own logo, but it must be a PNG file of 600x600 pixels and 3MB of maximum size."
												>
												<i class="fa fa-upload" aria-hidden="true"></i> Change logo</label>
												<input type="hidden" name="agentid" value="<?php echo $v["_id"]; ?>" />
											</div>
										</form>
										<?php } else { ?>
										<a href="applib/createLogo.php?agentid=<?php echo $v["_id"]; ?>" class="btn green agenttips"
										data-toggle="agenttip" data-trigger="hover" data-placement="bottom" title="Generates automatically a logo with the name of your agent."
										><i class="fa fa-file-image-o" aria-hidden="true"></i> Automatic logo</a>
										<br>
										<form action="applib/uploadLogoAgent.php" method="post" class="" id="uplogo_<?php echo $v["_id"]; ?>"  enctype="multipart/form-data">
											<div class="form-group" style="margin-top:10px;">
                ...
                <td style="position:relative" class="mt-element-ribbon">
					...
					<?php } else if(!$v["step1"]["status"] && $v["step1"]["agent_io_saved"]) { ?>
                        <div class="ribbon ribbon-clip" style="left:-20px;">
                ...
                <div style="margin: 10px 10px 10px 30px;">
					<?php if($v['last_status'] != "submitted" && $v['last_status'] != "registered" && $v['last_status'] != "rejected") { ?>
					  <?php if(!$v["step1"]["status"] && !$v["step1"]["agent_io_validated"] && !$v["step1"]["agent_io_saved"] && !$v["step1"]["agent_io_files"]) { ?>
					  ...
					  <?php } else { ?>
						...
						<?php if(!$v["step1"]["agent_io_validated"]) { ?>
                ...
                <!-- show step1 agent I/O -->
				<?php if($v["step1"]["agent_io_validated"] || $v["step1"]["agent_io_saved"] ) { ?>
                ...
                <div class="ribbon-content" style="clear:both; padding:10px 0px;">
					...
					<?php if($v["step2"]["agent_code"]) { ?>
												<input type="text" readonly class="form-control" value="<?php echo $v["step2"]["agent_code"]; ?>">
												<a href="<?php echo $v["step2"]["agent_code"]; ?>" target="_blank" class="input-group-addon agenttips" 
												data-toggle="agenttip" data-trigger="hover" data-placement="bottom" title="<?php echo $v["step2"]["agent_code"]; ?>"
                ...
                <div style="margin: 10px 10px 10px 30px;">
					<?php if($v['last_status'] != "submitted" && $v['last_status'] != "registered" && $v['last_status'] != "rejected") { ?>
					  <?php if(!$v["step3"]["status"] && !$v["step3"]["agent_spec_validated"] && !$v["step3"]["agent_spec_saved"]) { ?>
						<p><a class="btn btn-block btn-sm green" href="admin/jsonSpecValidator.php?id=<?php echo $v["_id"];?>"><i class="fa fa-plus" aria-hidden="true"></i> Define agent</a></p>
						<?php } else { 

						?>
						<p><a class="btn btn-block btn-sm green" href="admin/jsonSpecValidator.php?id=<?php echo $v["_id"];?>"><i class="fa fa-edit" aria-hidden="true"></i> Update agent specification</a></p>
                      <?php  } 
												} else { 
													if($_SESSION['User']['Type'] != 0) {
												?>
						<p><a class="btn btn-block btn-sm green" disabled><i class="fa fa-edit" aria-hidden="true"></i> Update agent specification</a></p>
												<?php } else { ?>
													<p><a class="btn btn-block btn-sm green" href="admin/jsonSpecValidator.php?id=<?php echo $v["_id"];?>"><i class="fa fa-edit" aria-hidden="true"></i> Update agent specification</a></p>
												<?php } ?>
                    <?php } ?>
                ...
                <!-- show step3 agent specification -->
                <div class="ribbon-content" style="clear:both; padding:10px 0px;">

				<?php if($v["step3"]["agent_spec"]) { ?>
					<p class="margin-top-10 margin-bottom-40"><a href="admin/viewJSON.php?id=<?php echo $v["_id"];?>&type=sp" target="_blank">View agent specification</a></p>
				<?php } else { ?>
					<p class="margin-top-10 margin-bottom-40"><a disabled>View agent specification</a></p>
				<?php } ?>
                ...
                <!-- show final step status -->
					<?php switch($v['last_status']) { 
						...
						case "in_preparation": 
                            if(!$v["step1"]["status"] || !$v["step2"]["status"] || !$v["step3"]["status"] ){ ?>
                                <div class="note note-warning">
                                    <p class="font-yellow"><b>IN PREPARATION</b>:<br/> Working on the agent..</p>
                                </div>
                            <?php } else { ?>
                                <div class="note note-warning">
                        ...
                        case "registered":  ?>
                            ...
                            <p class="font-white"><b>ACCEPTED</b>:<br/>Agent successfully registed!</p>
                            </div>
                            <?php break;
                ...
                <div style="margin: 10px 0;">
                    <?php if($v['last_status'] == "in_preparation"){ 
					    if(!$v["step1"]["status"] || !$v["step2"]["status"] || !$v["step3"]["status"] ){ ?>
                            <a class="btn btn-block btn-sm green" disabled><i class="fa fa-check" aria-hidden="true" title="Pass the previous steps before submitting the agent"></i> Submit agent</a>
                        <?php } else { ?>
                            <a class="btn btn-block btn-sm green" href="javascript:submitAgent('<?php echo $v["_id"];?>');"><i class="fa fa-check" aria-hidden="true"></i> Submit agent</a>
                    <?php } 
                    } ?>
                </div>
                  ...
                  <ul class="dropdown-menu pull-right" role="menu">
                  	<li><a href="applib/changeAgentStatus.php?agentid=<?php echo $v['_id']; ?>&status=in_preparation">In preparation</a></li>
                    <li><a href="applib/changeAgentStatus.php?agentid=<?php echo $v['_id']; ?>&status=submitted">Submitted</a></li>
										<li><a href="applib/changeAgentStatus.php?agentid=<?php echo $v['_id']; ?>&status=to_be_reviewed">To be reviewed</a></li>
										<li><a href="applib/changeAgentStatus.php?agentid=<?php echo $v['_id']; ?>&status=registered">Registered</a></li>
										<li><a href="applib/changeAgentStatus.php?agentid=<?php echo $v['_id']; ?>&status=rejected">Rejected</a></li>
                  </ul>
                </div>
								<?php } ?>
		...
		<div class="modal fade bs-modal" id="modalSubmitAgent" tabindex="-1" role="basic" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                                <h4 class="modal-title">Submit Agent</h4>
														</div>
														<form id="submit-agent" method="post" action="applib/submitAgent.php">
															<div class="modal-body table-responsive">
																<p id="st-title">You are about to submit the x agent, please fill the comments to send a message to our technical team</p>
																	<div class="form-group">
																		<textarea class="form-control" style="width:100%;" name="comments" rows="5" placeholder="Write your comments here"></textarea>
																	</div>
																	<input type="hidden" name="agentid" id="agentid-modal" value="" />
															</div>
								...
								<div class="modal fade bs-modal" id="modalRemoveAgent" tabindex="-1" role="basic" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                                <h4 class="modal-title">Remove Agent</h4>
														</div>
															<div class="modal-body table-responsive">
															...
															<div class="modal-footer">
																	<button type="button" class="btn dark btn-outline" data-dismiss="modal">Close</button>
																	<a href="javascript:;" id="btn-rmv-agent" class="btn red">Remove</a>
															</div>
                        </div>
                    </div>
                </div>
