var agentSelected = $("#agentSelected").val();
var table;
...
$(document).ready(function() {
  ...
  //pagingType: "full_numbers",
	...
	ordering: true,
	language: {
        emptyTable: 'No files found for the selected agent <i class="icon-question agenttips" data-container="body" data-html="true" data-placement="right" data-original-title="<p align=\'left\' style=\'margin:3px;\'>Please go to the <em>Get Data</em> section to load agent input files.</p><p align=\'left\' style=\'margin:3px;\'>More information on what this agent expects below on the <em>Agents\' Help</em> box, or in the main <em>Help</em> section</p>"></i>'
	},
	/*treetable: {
	  expandable: true
	},*/
 ...
 /* $('.mock_button').click(function(){
  ...
  // show / hide agents and buttons of the run agents portlet
  drawAgentsMenu = function(ch){
  	if(!ch && !checkIfSomeChecked()){
	  $('#desc-run-agents').show();
  	  $('#btn-av-agents').hide();
	  $('#btn-rmv-all').hide();
	}else{
	  $('#desc-run-agents').hide();
  	  $('#btn-av-agents').show();
	  $('#btn-rmv-all').show();
    }
  }

  // add / remove file to the run agents portlet
  drawAgentsList = function(ch, id, fl, fd, id_or, meta){
	if(ch){
	  var str_meta = '';
		meta = meta.replace(/"/g, "\'");
	  ...
	  $('#list-files-run-agents').append('<li class="agent-' + id + ' agent-list-item">'+
	  '<div class="col1">'+
		'<div class="cont">'+
        	'<div class="cont-col1">'+
	  ...
	  '<div class="col2">'+
		'<div class="label label-sm label-danger" style="float: right;padding:0">'+
            '<a href="javascript:removeFromAgentsList(\'agent-' + id  + '\', ' + id_or  + ');" title="Clear file from list" class="btn btn-icon-only red" style="width: 25px;height: 25px;padding-top: 1px;"><i class="fa fa-times-circle"></i></a>'+
        '</div>'+
      '</div>'+
	  '</li>');
  ...
  changeCheckbox = function(op) {
	...
	drawAgentsMenu(checked);
	drawAgentsList(checked, row.data('tt-id').toString().replace('.', ''), flname, fdname, row.data('tt-id').toString(), metadata);
  ...
  /*$('input[type=checkbox]', table.rows().nodes()).not('.foldercheck').change(function() {
	...
	drawAgentsMenu(checked);
	drawAgentsList(checked, row.data('tt-id').toString().replace('.', ''), flname, fdname, row.data('tt-id').toString());
	//console.log(JSON.stringify(allFiles));
  });*/
			...
			if((allFiles[i].folderId) == row.data('tt-id')) {
				if(checked) {
					if(allFiles[i].fileId !== undefined) {
						if(!allFiles[i].checked) drawAgentsList(checked, allFiles[i].rowId.toString().replace('.', ''), allFiles[i].fileName, allFiles[i].folderName, allFiles[i].rowId.toString(), allFiles[i].metadata);
						allFiles[i].checked = true;
					}
				}else{ 
					allFiles[i].checked = false;
					drawAgentsList(checked, allFiles[i].rowId.toString().replace('.', ''), allFiles[i].fileName, allFiles[i].folderName, allFiles[i].rowId.toString(), allFiles[i].metadata);
				}
			}
		}
		drawAgentsMenu(checked);
