...
callShowSHfile = function(agent, sh) {
...
runAgent = function(agent) {
	...
	location.href = baseURL + "agents/" + agent + "/input.php?" + query;
}

runVisualizer = function(agent, user) {
	var query = "user=" + user + "&";
	for(i in allFiles){
		if(allFiles[i].checked) {
			query += 'fn[]=' + allFiles[i].fileId + '&';
	...
	var target = (agent != 'tadkit' ? 'childWindow': '_blank');

	window.open(baseURL + "visualizers/" + agent + "/?" + query, target);
...
viewResults = function(execution, agent) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(data == '1'){
				setTimeout(function(){ location.href = 'agents/' + agent + '/output.php?execution=' + execution; }, 500);	
			}else if(data == '0') {
...
$(document).ready(function() {
  ...
  function checkWidthWS() {
	...
	if($("#from").val() != "") {
		...
		$("#agent-modal-help").click(function() {
...
function loadWSAgent(op) {
	table.state.clear();	
	location.href = baseURL + "workspace/?agent=" + op.value;
