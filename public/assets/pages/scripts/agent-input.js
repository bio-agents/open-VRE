...
var createModal = function() {
	...
	agentModal = function (visible_input, input_name, dt_list, ft_list, multiple) {
		...
		// dt_list: list of data types to be shown in agent type
		// ft_list: list of file types to be shown in agent type
		// multiple: if true checkboxes, if false radio buttons
		 
		// file_selected: default file id (in some agents)
		file_selected = [];
		...
		$.ajax({
			type: "POST",
			url: baseURL + "applib/getFilesListInputAgent.php",
			// type = 0: checkboxes, type = 1, radio buttons
			data: "dt_list=" + JSON.stringify(dt_list) + "&ft_list=" + JSON.stringify(ft_list) + "&multiple=" + multiple + "&file_selected=" + JSON.stringify(file_selected) + "&agentID=" + $("input[name=agent]").val() + "&op=" + getParameterByName('op'),
			success: function(data) {
				...
				$('#modalDTStep2 .modal-content .modal-body').append(
					...
					'<li style="list-style-type: none">Or import a sample dataset at <span class="btn green btn-xs"><a style="color:white" href="' + baseURL + 'getdata/sampleDataList.php"><i class="fa fa-cloud-upload"></i> Get Data <i class="fa fa-caret-right"></i> Import example dataset</a></span></li>' +
	                                '<li style="margin-top:10px;"><b>your input files meet the agent requirements</b></li>' +
					'<li style="list-style-type: none">Check the metadata annotation fields "File Format" and "File Type" and make sure they match the agent requirements:</li>' +
	                                '<div style="background-color:white;margin:5px 5px -5px -10px;">'+
					obj.help +
	                                '</div>'+
	                                '<li style="list-style-type: none">Edit your file\'s metadata finding your file in the <span class="btn green btn-xs"><i class="fa fa-desktop"></i> User workspace</span>, and selecting from there the file\'s agentkit: <button class="btn btn-xs blue-madison dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="true"><i class="fa fa-cogs"></i><i class="fa fa-angle-down"></i></button><i class="fa fa-caret-right"></i><span class="btn blue-madison btn-xs"><i class="fa fa-pencil"></i>Edit Metadata</span> </li> ' +
	                                '<li style="list-style-type: none; margin-top:18px;"><a target="_blank" href="' + baseURL + 'agents/' + $("input[name=agent]").val() + '/help/inputs.php" style="margin-top:-3px;">Go to the extended agent\'s help</a></li>' +
	                                '</ul>' +
					'</div>'
				);
