...
//var_dump($_SESSION['validation']);
	...
	case 'BAM':
      ...
      if (count($subs) || $sort || $index || $chrs){
        ...
        // enqueue BAMval agent	
		$resp['state']=3;
		$agentId = "BAMval";
		//agent inputs -- should match agent registry
		$agentInputs = array("bam"=>array($fn));
		//agent arguments -- should match agent registry
        $agentArgs   = array(
                    "sort"    => $sort,
				    "index"   => $index,
				    "replace" => "\"".join(" | ",$subs) ."\""
				   );
		//agent outputs -- metadata saved in mongo until agent output registration phase
        ...
        $baiOut=array(
				...
				"taxon_id" => $_REQUEST['taxon_id'],
				"meta_data"=> $baiMeta);
		$agentOuts = Array ("output_files" => Array($bamOut, $baiOut));
		//agent logName
		$logName = basename($bamFn,".bam"). ".log"; 
		//call agent
		$pid = launchAgentInternal("BAMval",$agentInputs,$agentArgs,$agentOuts,$output_dir,$logName);
