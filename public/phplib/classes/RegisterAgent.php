...
class RegisterAgent {
    ...
    /**
     * Creates new agent Register process
     * @param string $agent Agent object as appears in Mongo
    */
    public function __construct($agent_io,$execution=0,$agent_executable=0,$agent_lib=0){
    
    	// Setting RegisterAgent
    	$this->agent_io   = $agent_io;
        $this->agentId    = $agent_io['_id'];
        ...
        $this->agent_lib        =($agent_lib? $agent_lib : "/packages/to/add/into/pythonpath");
        $this->agent_executable =($agent_executable? $agent_executable : "/main/mg-agent/executable.py");
        $this->execution       =($execution? $execution : "/test/execution/directory");
        if (!preg_match('/^\//', $execution) || !preg_match('/^\//', $agent_lib) || !preg_match('/^\//', $agent_executable) ){
        ...
        // Create working_dir
        $this->dev_dir = $GLOBALS['dataDir']."/".$_SESSION['User']['id']."/.dev/".$this->agentId;
        $this->working_dir = $GLOBALS['dataDir']."/".$_SESSION['User']['id']."/.dev/".$this->agentId."/test";
    ...
    */
    ...
    // check agent_io
    if (!isset($this->agent_io['_id']) || !isset($this->agent_io['input_files']) || !isset($this->agent_io['arguments']) || !isset($this->agent_io['output_files'])){
    ...
    $c=0;
    foreach ($this->agent_io['input_files_combinations'] as $comb){
        ...
        // append input_files
        foreach ($this->agent_io['input_files'] as $input_name=>$input){
            ...
            if (in_array($input_name,$comb['input_files']) ){
                foreach ($this->input_files[$input_name] as $v){
                    array_push($data['input_files'], Array(
                                            "name"          => $input_name,
                                            "value"         => $v,
                                            "required"      => $this->agent_io['input_files'][$input_name]['required'],
                                            "allow_multiple"=> $this->agent_io['input_files'][$input_name]['allow_multiple']
                                     )
                    );
                }
           }
        ...
        if (count($this->agent_io['input_files_public_dir'])){
          foreach ($this->agent_io['input_files_public_dir'] as $input_name=>$input){
            ...
            if (isset($comb['input_files_public_dir']) && in_array($input_name,$comb['input_files_public_dir'])){
                foreach ($this->input_files_pub[$input_name] as $v){
                    array_push($data['input_files'], Array(
                                            "name"          => $input_name,
                                            "value"         => $v,
                                            "required"      => $this->agent_io['input_files_public_dir'][$input_name]['required'],
                                            "allow_multiple"=> $this->agent_io['input_files_public_dir'][$input_name]['allow_multiple']
                                     )
                    );
                }
           }
        ...
        if ($this->agent_io['output_files']){
            foreach ($this->agent_io['output_files'] as $k => $v){
    ...
    */
	...
	foreach ($arguments as $arg_name => $arg_value){
		...
		if ($arg_value==""){
		    if ($agent_io['arguments'][$arg_name]['required']){
		...
		switch ($agent_io['arguments'][$arg_name]['type']){
		    case "enum":
			if (!isset($agent_io['arguments'][$arg_name]['enum_items']) || (!isset($agent_io['arguments'][$arg_name]['enum_items']['name']))){
			    $_SESSION['errorData']['Internal'][]="Invalid argument enum in agent definition. '$arg_name' has no 'enum_items' or 'enum_items['name].";
		 	    return 0;
			}
			if (!in_array($arg_value,$agent_io['arguments'][$arg_name]['enum_items']['name']) ){
	    		    $_SESSION['errorData']['Error'][]="Invalid argument. In '$arg_name' these values are accepted [".implode(", ",$agent_io['arguments'][$arg_name]['enum_items']['name'])."], but found $arg_value";
			    return 0;
			}
			break;
		    case "enum_multiple":
			...
			foreach ($arg_value as $v){
				if (!in_array($v,$agent_io['arguments'][$arg_name]['enum_items']['name']) ){
			    		$_SESSION['errorData']['Error'][]="Invalid argument. In '$arg_name' these values are accepted [".implode(", ",$agent_io['arguments'][$arg_name]['enum_items']['name'])."], but found  ".implode(", ",$arg_value);
					return 0;
				}
			}
			break;
     ...
     * @param array $agent_io Agent array containing input_files type and requirements
    */
    public function setMetadata_fromAgent($input_file_paths,$user_metadata=array(),$is_public=false){
        ...
        foreach ($input_file_paths as $input_name => $file_paths){
            ...
            foreach ($file_paths as $file_path){
                if (isset($agent_io[$input_files_key][$input_name])){
                    $fileInput = array("file_type" => $agent_io[$input_files_key][$input_name]["file_type"][0],
                                       "data_type" => $agent_io[$input_files_key][$input_name]["data_type"][0],
                                       "path" => $file_path,
                                       "project" => $this->project);
    ...
    */
	...
	foreach ($input_file_paths as $input_name => $file_paths){
	    //checking  requirements
	    if (count($agent_io) && count($metadata)){
		    if (!is_array($file_paths))
    			$file_paths=array($file_paths);
...
//      }
    ...
    */
    ...
    foreach ($this->agent_io['input_files_combinations'] as $comb){
    	...
    	fwrite($fout, "TOOL_EXECUTABLE=".$this->agent_executable."\n");
    	if ($this->agent_lib && preg_match('/COMPSs/i',$workflowtype)){fwrite($fout, "TOOL_LIBRARY=".$this->agent_lib."\n");}
    	fwrite($fout, "\n");
        ...
        fwrite($fout, "\n");
        fwrite($fout, "time $cmd > \$WORKING_DIR/agent.log\n");
        fwrite($fout, "\n");
        fclose($fout);
    ...
    */
    public function save_test_files(){
        ...
        $GLOBALS['agentsDevMetaCol']->updateOne(array('_id' => $this->agentId),
                                            array('$set'   => array('last_status_date' => date('Y/m/d H:i:s'),
                                                                    'step1.date'       => date('Y/m/d H:i:s'),
                                                                    'step1.agent_io_files' => true,
                                                                    'step1.status'        => true,
