...
class Agentjob {
    ...
    public $cloudName;         // Cloud name where agent should be executed. Available clouds set in GLOBALS['clouds']
    public $description;
    public $working_dir;
    public $output_dir;
    public $launcher;
    ...
    // Paths to files genereted during AgentJob execution
    public $config_file;
    ...
    /**
     * Creates new agentExecutor instance
     * @param string $agentId Agent Id as appears in Mongo
    */
    public function __construct($agent,$execution="",$project="",$descrip="",$output_dir=""){
    

    	// Setting Agentjob
    	$this->agentId    = $agent['_id'];
    	$this->title     = $agent['name'] ." job";
        ...
        $this->set_cloudName($agent);
        $this->launcher         = $agent['infrastructure']['clouds'][$this->cloudName]['launcher'];
        switch ($this->launcher){
            ...
            case "PMES":
                ...
                break;
            default:
                $_SESSION['errorData']['Error'][]="Agent '$this->agentId' not properly registered. Launcher type is set to '".$this->launcher."'. Case not implemented.";
        }
        ...
        if ($execution != "0"){
            ...
            $this->__setWorking_inTmp($agent['_id']);
            $this->output_dir = $output_dir;
        }
    
        // Set description
        if ($descrip != "")
    		$this->setDescription($descrip,$agent['name']);
    ...
    /**
     * Fetch agent entry in Mongo 
     * @param string $agentId Agent Id as appears in Mongo
    */
    protected function getAgent($agentId){
	$agent   = $GLOBALS['agentsCol']->findOne(array('_id' => $agentId));
	if (empty($agent)){
                $_SESSION['errorData']['Agentjob'][]="Agent '$agentId' is not registered. Cannot submit execution. Please, contact <a href=\"mailto:".$GLOBALS['helpdeskMail']."\">us</a>";
                return 0;
	}
	//$this->agent= (object) $agent;
	$this->agent= $this->array_to_object($agent);
    }
    ...
    */
    public function setDescription($descrip,$agentName=0){
        if (strlen($descrip))
                $this->description=$descrip;
	elseif($agentName)
                $this->description="Execution directory for agent ".$agentName;
	else
    ...
    public function setLog($filename=""){
        ...
        if (strlen($filename)){
    		...
    		$this->logName = $GLOBALS['agent_log_file'];
        }
        ...
        if ($this->hasExecutionFolder){
            ...
            $this->__setWorking_inTmp($this->agentId);
        }
    }
    ...
    */
    ...
    public function __setWorking_dir($execution, $overwrite=0){
        ...
        if (!$this->logName){$this->logName = $GLOBALS['agent_log_file'];}

    	$this->config_file    = $this->working_dir."/".$GLOBALS['agent_config_file'];
    	$this->stageout_file  = $this->working_dir."/".$GLOBALS['agent_stageout_file'];
        $this->submission_file= $this->working_dir."/".$GLOBALS['agent_submission_file'];
        $this->log_file       = $this->working_dir."/".$this->logName;
        $this->metadata_file  = $this->working_dir."/".$GLOBALS['agent_metadata_file'];

        $this->config_file_virtual    = $this->root_dir_virtual."/".$this->project."/".$this->execution."/".$GLOBALS['agent_config_file'];
        $this->stageout_file_virtual  = $this->root_dir_virtual."/".$this->project."/".$this->execution."/".$GLOBALS['agent_stageout_file'];
        $this->metadata_file_virtual  = $this->root_dir_virtual."/".$this->project."/".$this->execution."/".$GLOBALS['agent_metadata_file'];
    ...
    public function __setWorking_inTmp($prefixDir=0){
        ...
        $this->config_file_virtual    = $this->root_dir_virtual."/".$this->project."/".$GLOBALS['tmpUser_dir'].$this->execution."/".$GLOBALS['agent_config_file'];
        $this->stageout_file_virtual  = $this->root_dir_virtual."/".$this->project."/".$GLOBALS['tmpUser_dir'].$this->execution."/".$GLOBALS['agent_stageout_file'];
        $this->metadata_file_virtual  = $this->root_dir_virtual."/".$this->project."/".$GLOBALS['tmpUser_dir'].$this->execution."/".$GLOBALS['agent_metadata_file'];
    ...
    */
    ...
    /**
     * Creates agent configuration JSON
     * @param array $agent Fill in config file: input_files, arguments and output_files
    */
    public function setConfiguration_file($agent){
	
	$config_rfn = $this->config_file;

	if (!$this->working_dir){
		$_SESSION['errorData']['Internal Error'][]="Cannot create agent configuration file. No 'working_directory' set";
		return 0;
	}
	...
	foreach ($this->input_files as $k=>$vs){
	    foreach ($vs as $v){
            array_push($data['input_files'], Array(
                                                "name"          => $k,
                                                "value"         => $v,
                                                "required"      => $agent['input_files'][$k]['required'],
                                                "allow_multiple"=> $agent['input_files'][$k]['allow_multiple']
                                            )
                       );
	   }
	}
	foreach ($this->input_files_pub as $k=>$vs){
	    foreach ($vs as $v){
            array_push($data['input_files'], Array(
                                                "name"          => $k,
                                                "value"         => $v,
                                                "required"      => $agent['input_files_public_dir'][$k]['required'],
                                                "allow_multiple"=> $agent['input_files_public_dir'][$k]['allow_multiple']
                                            )
                       );
	   }
	}
    ...
    // append output_files from agent json
    if ($agent['output_files']){
        foreach ($agent['output_files'] as $k => $v){
	...
	try{
	    ...
	    if (!$F) {
		throw new Exception('Failed to create agent configuration file'.$config_rfn);
	    }
    	}
	catch (Exception $e){
    ...
    */
    public function setArguments($arguments,$agent=Array()){

	foreach ($arguments as $arg_name => $arg_value){
	    //checking  requirements
	    if (count($agent)){
		// checking coherence between JSON and REQUEST
		if (!isset($agent['arguments'][$arg_name])){
			$_SESSION['errorData']['Internal'][]="Argument '$arg_name' not found in agent definition. '$this->agentId' is not properly registered";
			return 0;
		}
		// checking arguments requirements (TODO create 'validateArguments')
		if ($arg_value==""){
		    if ($agent['arguments'][$arg_name]['required']){
		...
		switch ($agent['arguments'][$arg_name]['type']){
		    case "enum":
			if (!isset($agent['arguments'][$arg_name]['enum_items']) || (!isset($agent['arguments'][$arg_name]['enum_items']['name']))){
			    $_SESSION['errorData']['Internal'][]="Invalid argument enum in agent definition. '$arg_name' has no 'enum_items' or 'enum_items['name].";
		 	    return 0;
			}
			if (!in_array($arg_value,$agent['arguments'][$arg_name]['enum_items']['name']) ){
	    		    $_SESSION['errorData']['Error'][]="Invalid argument. In '$arg_name' these values are accepted [".implode(", ",$agent['arguments'][$arg_name]['enum_items']['name'])."], but found $arg_value";
			    return 0;
			}
			break;
		    case "enum_multiple":
			...
			foreach ($arg_value as $v){
				if (!in_array($v,$agent['arguments'][$arg_name]['enum_items']['name']) ){
			    		$_SESSION['errorData']['Error'][]="Invalid argument. In '$arg_name' these values are accepted [".implode(", ",$agent['arguments'][$arg_name]['enum_items']['name'])."], but found ".implode(", ",$arg_value);
					return 0;
				}
			}
			break;
		    ...
		    case "enum":
		    default:
			$_SESSION['errorData']['Internal'][]="Invalid argument type in agent definition. '$arg_name' is of type ".$agent['arguments'][$arg_name]['type'];
		 	return 0;
		}
	    }
	    // setting arguments 
     ...
     * @param array $agent Agent array containing input_files type and requirements
     * @param array $metadata Files metadata extracted from DB
    */
    public function setInput_files($input_files,$agent=array(),$metadata=array()){

	foreach ($input_files as $input_name => $fns){

	    //checking  requirements
	    if (count($agent) && count($metadata)){
		    if (!is_array($fns))
			$fns=array($fns);

            foreach ($fns as $fn){
			// checking coherence between JSON and REQUEST
			if (!isset($agent['input_files'][$input_name])){
				$_SESSION['errorData']['Internal'][]="Input file '$input_name' not found in agent definition. '$this->agentId' is not properly registered";
				return 0;
            } 
            // checking required value not empty
			if (!$fn){
			    if ($agent['input_files'][$input_name]['required'] === true ){
...
/*			$ok = $this->validateInput_file($agent['input_files'][$input_name], $metadata[$fn]);
			if (! $ok){
				$_SESSION['errorData']['Error'][]="Input file '$input_name' not valid. Stopping '$this->agentId' execution";
				return 0;
            }*/
		    }   
	    }
    ...
    /*
    if (count($agent['input_files'])){
        foreach ($agent['input_files'] as $input_name => $input){
    ...
    */
    ...
    public function setInput_files_public($input_files_public,$agent=array(),$metadata_pub=array()){
    foreach ($input_files_public as $input_name => $input_values){

        $fns = array();
        //checking  requirements
        if (count($agent) && count($metadata_pub)){
		...
		// checking coherence between JSON and REQUEST
		if (!isset($agent['input_files_public_dir'][$input_name])){
			$_SESSION['errorData']['Internal'][]="Input file public '$input_name' not found in agent definition. '$this->agentId' is not properly registered";
			return 0;
            }
    	    ...
    	    // checking input_file integrity
/*	        $ok = $this->validateInput_file($agent['input_files_public'][$input_name], $metadata_pub[$fn]);
		    if (! $ok){
			   	$_SESSION['errorData']['Error'][]="Input file public '$input_name' not valid. Stopping '$this->agentId' execution";
                return 0;
            }
            */
            }
    ...
    /**
     * Store its metadata in Agentjob for recovering it latter, while stageout register
     * Needed when agent has not APP (internal), and no out_metadata is generated. 
     * @param array $outs Array of outputfiles
     * @param array $agent Agent array containing input_files type and requirements TODO
     * @param array $metadata Files metadata extracted from DB TODO
    */
    public function setStageout_data($out_files,$agent=array(),$metadata=array()){
	if (!isset ($out_files['output_files'])){
		$_SESSION['errorData']['Error'][]="Internal agent may have problems registering outfiles: Stageout_data mal formatted";
		return 0;
	}

	foreach ($out_files['output_files'] as $out_name => $info){
		//Validate out_files against agent document
		//TODO
     ...
     * @param array $inputReq  Input_file as defined in agent collection (derived from agent JSON definition)
    ...
    */
	...
	if (!isset($inputReq['file_type']) && !isset($inputReq['data_type']) ){
		$_SESSION['errorData']['Warning'][]="Ommitting format and type control for input file '".$inputReq['name'].". Agent has no 'file_type' nor 'data_type' set.";
		return 1;
	}
    ...
    */
	...
	try{
	    ...
	    if (!$F) {
		throw new Exception('Failed to create metadata file for agent execution'.$metadata_rfn);
	    }
    	}
	catch (Exception $e){
		$_SESSION['errorData']['Internal Error'][]= $e->getMessage();
    ...
    */
    public function prepareExecution($agent,$metadata,$metadata_pub = array()){

	$launcher = $agent['infrastructure']['clouds'][$this->cloudName]['launcher'];

    //external agent
    if ($agent['external'] !== false){

		$r = $this->setConfiguration_file($agent);
		if ($r=="0")
		    return 0;
		...
		if (!is_file($this->config_file) && !is_file($this->metadata_file) ){
			$_SESSION['errorData']['Internal Error'][]="Cannot set agent command line. It required configuration file ($this->config_file) and metadata file ($this->metadata_file)";
	            	return 0;
		}

		switch ($launcher){
		    case "SGE":
			$cmd  = $this->setBashCmd_SGE($agent);
			if (!$cmd)
				return 0;
		    ...
		    case "PMES":
			$json_data = $this->setPMESrequest($agent);
			if (!$json_data)
				return 0;
		    ...
		    default:
			$_SESSION['errorData']['Error'][]="Agent '$this->agentId' not properly registered. Launcher for '$this->agentId' is set to \"$launcher\". Case not implemented.";
			return 0;
		}
	...
	//internal agent
	}elseif ($agent['external'] === false){

		switch ($launcher){
	
		    case "SGE":
			$cmd = $this->setBashCmd_withoutApp($agent,$metadata);
			if (!$cmd)
				return 0;
		    ...
		    default:
			$_SESSION['errorData']['Error'][]="Internal Agent '$this->agentId' not properly registered. Launcher for '$this->agentId' is set to \"$launcher\". Case not implemented.";
			return 0;
		}
		return 1;
	}
    ...
    protected function setBashCmd_SGE($agent){
	if (!isset($agent['infrastructure']['executable'])){
            $_SESSION['errorData']['Internal Error'][]="Agent '$this->agentId' not properly registered. Missing 'executable' property";
            return 0;
	}
	$cmd = $agent['infrastructure']['executable'] .
    ...
    protected function setPMESrequest($agent){
	...
	//Setting defaults from agent definition 
	if (!isset($agent['infrastructure']['wallTime']) )
	   $agent['infrastructure']['wallTime'] = "1440"; // 24h

    if (!isset($agent['infrastructure']['interpreter'])){
        $agent['infrastructure']['interpreter']="";  // only required if "Single". Examples:  "bash", "python3" 
    }
    $cloud   = $agent['infrastructure']['clouds'][$this->cloudName];
	...
	if (!isset($cloud['imageType']) ){
        ...
        $flavor = $this->setImageType($agent['infrastructure']['cpus'],$agent['infrastructure']['memory']);
        $cloud['imageType']                 = $flavor['id'];
		$agent['infrastructure']['memory']   = $flavor['memory'];
		$agent['infrastructure']['cpus']     = $flavor['cpus'];
	...
	$app_target =  dirname($agent['infrastructure']['executable']);
    $app_source = basename($agent['infrastructure']['executable']);
	...
	$data = array(
        ...
        "wallTime"   => $agent['infrastructure']['wallTime'], 
		"memory"     => $agent['infrastructure']['memory'],
		"cores"      => $agent['infrastructure']['cpus'],
		...
		"img"        => array(
      	...
      	"app"        => array(
				...
				"target" => $app_target,
                "source" => $app_source,
                "interpreter" => $agent['infrastructure']['interpreter'],
				"args"  => array(
					"config"      => $this->config_file_virtual,
    ...
    protected function setBashCmd_withoutApp($agent,$metadata){
	if (!isset($agent['infrastructure']['executable'])){
            $_SESSION['errorData']['Internal Error'][]="Agent '$this->agentId' not properly registered. Missing 'executable' property";
            return 0;
	}
	...
	try{
	    ...
	    if (!$fout) {
		throw new Exception('Failed to create agent configuration file: '.$bash_rfn);
	    }
    	}
	catch (Exception $e){
	...
	try{
	    ...
	    if (!$fout) {
		throw new Exception('Failed to create agent configuration file: '.$json_rfn);
	    }
    	}
	catch (Exception $e){
		$_SESSION['errorData']['Error'][]="Failed to create queue submission file. ".$e->getMessage();
    ...
    */
    public function submit($agent){
	    switch ($agent['infrastructure']['clouds'][$this->cloudName]['launcher']){
    	    case "SGE":
        		...
        		break;
    	    default:
    	    	$_SESSION['errorData']['Error'][]="Agent '$this->agentId' not properly registered. Launcher for '$this->agentId' is set to: \"".$agent['infrastructure']['clouds'][$this->cloudName]['launcher']."\". Case not implemented.";
    		    return 0;
    	}
	    return 1;
    }	    
    ...
    protected function enqueue($agent){
	
    	logger("");
    	$memory = $agent['infrastructure']['memory'];
    	$cpus   = $agent['infrastructure']['cpus'];
    	$queue  = $agent['infrastructure']['clouds'][$this->cloudName]['queue'];
    
    	list($pid,$errMesg) = execJob($this->working_dir, $this->submission_file, $queue, $cpus, $memory);
        if (!$pid){
            log_addError($pid,$errMesg,NULL, $this->agentId,$this->cloudName,"SGE",$cpus,$memory);
            $_SESSION['errorData']['Error'][]="Internal error. Cannot enqueue job.";
            return 0;
        }
        logger("USER:".$_SESSION['User']['_id'].", ID:".$_SESSION['User']['id'].", LAUNCHER:SGE, TOOL:".$this->agentId.", PID:$pid");
        log_addSubmission($pid,$this->agentId,$this->cloudName,"SGE",$cpus,$memory,$this->working_dir);
    ...
    protected function callPMES(){
    	...
    	if (!$pid){
            log_addError($pid,$errMesg,NULL,$this->agentId,$this->cloudName,"PMES",$data['cores'],$data['memory']);
            $_SESSION['errorData']['Error'][]="Internal error. Cannot enqueue job.";
            return 0;
        }

    	logger("USER:".$_SESSION['User']['_id'].", ID:".$_SESSION['User']['id'].", LAUNCHER:PMES, TOOL:".$this->agentId.", PID:$pid");
        log_addSubmission($pid,$this->agentId,$this->cloudName,"PMES",$data[0]['cores'],$data[0]['memory'],$this->working_dir);
    ...
    */
    ...
    /**
    *  Set Cloudname to the default value, as specified in the agent definition
    *  TODO Choose cloud according where the data is.
    */
    protected function set_cloudName($agent=array()){
	...
	if (isset($agent['infrastructure']['clouds'])){
		// 1, set cloudName from default cloud, as agent specifies
		foreach ($agent['infrastructure']['clouds'] as $name=>$agentInfo){
			if ($agentInfo['default_cloud'] === true){
        ...
        // 2, set cloudName from current cloud, if it is in agent specification
        ...
        // 3, set cloudName from clouds list in agent specification, the first found available
		if (! $this->cloudName){
			foreach ($agent['infrastructure']['clouds'] as $name=>$cloudInfo){
				if (in_array($name,$available_clouds)){
					$this->cloudName = $name;
					$_SESSION['errorData']['Warning'][] = "Agent has no the default cloud infrastructure set or available. Taking instead '$this->cloudName', but the agent execution may fail.";
					break;
				}
			}
		}
	...
	if (! $this->cloudName){
		...
		$_SESSION['errorData']['Warning'][] = "Agent has no the cloud infrastructure set. Taking '$this->cloudName', but the agent execution may fail.";
	}
	return 1;
    }
     ...
     * @param array $metadata Files metadata extracted from DB
    */
    public function createMetadata_from_Input_files_public($input_files_public,$agent){
	    ...
	    foreach ($input_files_public as $input_name => $input_value){
            ...
            if ($input_value!=""){
                ...
                switch ($agent['input_files_public_dir'][$input_name]['type']){
                    case 'enum':
    			        if (!isset($agent['input_files_public_dir'][$input_name]['enum_items']) || (!isset($agent['input_files_public_dir'][$input_name]['enum_items']['name']))){
                            $_SESSION['errorData']['Internal'][]="Invalid input_files_public_dir enum in agent definition. '$input_name' has no 'enum_items' or 'enum_items['name].";
                            $rfn_public = 0;
                        }
			            if (!in_array($input_value,$agent['input_files_public_dir'][$input_name]['enum_items']['name']) ){
	    		            $_SESSION['errorData']['Error'][]="Invalid input_files_public_dir. In '$input_name' these values are accepted [".implode(", ",$agent['input_files_public_dir'][$input_name]['enum_items']['name'])."], but found $input_value";
                            $rfn_public = 0;
                        }
                    ...
                    case 'string':
            			...
            			break;
                    default:
                        $_SESSION['errorData']['Internal'][]="Input file public '$input_name' has unsupported type (".$agent['input_files_public_dir'][$arg_name]['type']."). '$this->agentId' is not properly registered";
                        $rfn_public = 0;
                }
                if ($rfn_public == 0 ){
                    continue;
                ...
                if (isset($agent['input_files_public_dir'][$input_name]['data_type']) && is_array($agent['input_files_public_dir'][$input_name]['data_type'])){
                    $file['data_type']= $agent['input_files_public_dir'][$input_name]['data_type'][0];
                }
                if (isset($agent['input_files_public_dir'][$input_name]['file_type']) && is_array($agent['input_files_public_dir'][$input_name]['file_type'])){
                    $file['file_type']= $agent['input_files_public_dir'][$input_name]['file_type'][0];
                }
                $file['user_id']= "public";
    ...
    /**
     * Assign agent VM size (image type) according the demanded CPUS and RAM 
