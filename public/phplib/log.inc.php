...
function log_addSubmission($pid,$agentId,$cloudName,$launcher,$cpus,$memory,$wd,$test=FALSE){
...
function log_addError($pid,$msg,$errCode=0, $agentId=FALSE,$cloudName=FALSE,$launcher=FALSE,$cpus=FALSE,$memory=FALSE,$test=FALSE){
    ...
    if ($agentId)   {$log_exec_extra['agentId']   = $agentId;}
...
function getStatsFromJobLogs($jobs,$byAgents=array("all")){
    ...
    // initialize
    $stats = array();
    foreach ($byAgents as $agentId){
	    $stats[$agentId] = array("jobs_total" => 0,
    ...
    foreach ($jobs as $pid => $j){
        ...
        //filter by agentId
        $agentId = $j['agentId'];
        if (!in_array($agentId,$byAgents) && $byAgents[0] != "all")
                continue;
        if ($byAgents[0] == "all" && count($byAgents)==1 ){
            $agentId = "all";
        }

        // count total jobs
        $stats[$agentId]["jobs_total"]++;

        // count err/success jobs 
        if ($j["success"] == "TRUE"){ $stats[$agentId]['jobs_finished_success']++; }
        if ($j["success"] == "ERR") { $stats[$agentId]['jobs_finished_err']++ ; }
        if ($j["success"] == "")    { $stats[$agentId]['jobs_finished_unk']++ ; }

        // count distinct user
        $stats[$agentId]['distinct_users'][$j['user']]=1;

        // comp average execution time
        $duration = number_format(($j["timestamp_end"] - $j["timestamp_start"])/60,2); 
        $stats[$agentId]['avg_duration'] = ( $stats[$agentId]['avg_duration']? ($stats[$agentId]['avg_duration'] + $duration)/2 : $duration);

        // count last mouth frequency execution
        if(isset($j["date_start"])){
            $monthyear = date( 'Y/m' ,$j['timestamp_start']);
            if(array_key_exists($monthyear, $stats[$agentId]["freq_executions"])) $stats[$agentId]["freq_executions"][$monthyear]++;
    	}
    }
