...
function getChrFromBAM($bamFn){
    $dirTmp   = $GLOBALS['dataDir']."/".$_SESSION['User']['id']."/".$_SESSION['User']['activeProject']."/".$GLOBALS['tmpUser_dir'];
	$samagents = "/orozco/services/Rdata/Web/apps/samagents/bin/samagents";
	$SQs      = "";
	$stdErr   = "";
	$chrNames = Array();

	# check chromosome names from header
	subprocess("$samagents view -H '$bamFn' | grep \"\@SQ\"",$SQs,$stdErr,$dirTmp);
	if ($stdErr){
		$_SESSION['errorData']['error'][]="Error extracting chromosome names: $stdErr";
		return (false);
	}
...
//	if (count($chrNames)==0){
//		subprocess("$samagents view '$bamFn' | cut -f3 | sort -u",$SQs,$stdErr,$dirTmp);
//		if ($stdErr){
//			$_SESSION['errorData']['error'][]="Error extracting chromosome names: $stdErr";
//			return (false);
//		}
...
function validateBAM($bamFn,$refGenome){
	...
	if (count($chrs)==0){
		...
		#NOTE: An alternative is to get chrs from 'samagents idxstat aln.sorted.bam'
	}
	if (count($chrs) == 0){
...
function queueBAMvalidation ($prepNum,$tmpdir,$outdir,$bamFn,$type,$sort,$subs,$index,$cores) {
	...
	fwrite($fout, "$samagents view '$bamFn' | cut -f3 | sort -u >> $outdir/PP_$prepNum.log 2>&1\n");
    ...
    if (count($subs) || $sort){
        if (count($subs)){
            ...
            fwrite($fout, "$samagents view -h $bamFn |". join(" | ",$subs) . "|". "$samagents view -uhS - | $samagents sort - -o $bamTmp >> $outdir/PP_$prepNum.log 2>&1\n");
            fwrite($fout, "echo '# End time:' \$(date) >> $outdir/PP_$prepNum.log\n");
        }else{
            fwrite($fout, "\necho \"#Sorting BAM file...\" >  $outdir/PP_$prepNum.log\n");
            fwrite($fout,"$samagents sort $bamFn -o $bamTmp >> $outdir/PP_$prepNum.log 2>&1\n");
            fwrite($fout, "echo '# End time:' \$(date) >> $outdir/PP_$prepNum.log\n");
        }
    ...
    if (count($subs) || $sort || $index){
        ...
        fwrite($fout,"$samagents index $bamFn >> $outdir/PP_$prepNum.log\n");
        fwrite($fout, "echo '# End time:' \$(date) >> $outdir/PP_$prepNum.log\n");
