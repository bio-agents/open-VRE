...
function getData_fromURL($url, $meta = null, $uploadType="url"){
    if ($uploadType == "id"){
       list($agentArgs,$agentOuts,$output_dir) = prepare_getData_fromURL($url,"uploads",$GLOBALS['BASEURL']."/getdata/dataFromID.php",$meta);
       getData_wget_syncron($agentArgs,$agentOuts,$output_dir,"uploads",$GLOBALS['BASEURL']."/getdata/dataFromID.php"); 
    }else{
       list($agentArgs,$agentOuts,$output_dir) = prepare_getData_fromURL($url,"uploads",$GLOBALS['BASEURL']."/getdata/uploadForm.php#load_from_url",$meta);
       getData_wget_asyncron($agentArgs,$agentOuts,$output_dir,$GLOBALS['BASEURL']."/getdata/uploadForm.php#load_from_url"); 
    }
    echo 1;
}
...
function prepare_getData_fromURL($url,$outdir,$referer,$meta=array()) {
    ...
    if ($fnId){
        ...
        // setting agent	arguments
        $agentArgs  = array(
                "url"    => $url,   
                "output" => $fnP);           // Agent is responsible to create outputs in the output_dir
        if ($url_withCredentials){
            $agentArgs["url"] = $url_withCredentials;
        }
        // setting agent outputs -- metadata to save in DMP during agent output_file registration
        ...
        $agentOuts = Array ("output_files" => Array($fileOut));
        
    }
    return array($agentArgs,$agentOuts,$output_dir);
}



//function getData_wget($url,$outdir,$referer,$meta=array()) {
function  getData_wget_asyncron($agentArgs,$agentOuts,$output_dir,$referer){
 
   // choosing interanl agent 
   $agentId = "wget";	

    // setting agent	inputs
   $agentInputs= array();

   // setting logName
   $fnP = $agentOuts['output_files'][0]["file_path"];
   $logName = basename($fnP). ".log";


   //asyncronous download file (internal agent wget)
   //FIXME START - This is a temporal fix. In future, files should not be downloaded, only registered
   $pid = launchAgentInternal($agentId,$agentInputs,$agentArgs,$agentOuts,$output_dir,$logName);
...
function  getData_wget_syncron($agentArgs,$agentOuts,$output_dir,$referer){
   ...
   // run agent executable interactively
   if ($agent){
        $executable = $agent['infrastructure']['executable'];
        ...
        $msg ="File imported from URL '".basename($fnP)."' cannot be imported. Cannot find agent 'Get remote file'";
        if($referer == "die"){die($msg);}else{$_SESSION['errorData']['Error'][] =$msg; redirect($referer);}
   }
...
print "output  (file or folder) = $output\n";
    ...
    //Do asyncronous download file (internal agent wget)
        ...
        // setting agent	arguments. Agent is responsible to create outputs in the output_dir
        $agentArgs  = array(
                "url"    => $url,   
                "output" => $fnP
	);
	if ($compression and $extract_uncompress){
		...
		foreach($compressors as $c){
		    if ($c == "TAR"){
			$agentArgs['archiver']  = $c;
			continue;
		    }else{
			$agentArgs['compressor']  = $c;
			continue;
		    }
		}
//			$agentArgs['uncompress_cmd'] =  " tar -xz -C ";
//			$agentArgs['uncompress_cmd'] =  " tar -xj ";
//			$agentArgs['uncompress_cmd'] =  " gunzip -c ";
//			$agentArgs['uncompress_cmd'] =  " bzip2 ";
	}

        // setting agent output metadata. It will be registerd after agent execution
        if (!$descrip){
...
*/
    ...
    if ($fnId){
        ...
        // setting agent outputs. Metadata will be saved in DB during agent output_file registration
        if (!$descrip){
		$descrip="Remote file extracted from <a target='_blank' href=\"$url\">$url</a>";
	}
        if (!$filetype){
...
function getSampleDataList($status=1,$filter_agent_status=true) {

    $ft;
    if ($filter_agent_status){
	    $fa = indexArray($GLOBALS['agentsCol']->find(array('status' => 1),array('_id' => 1)));
	    $fu = indexArray($GLOBALS['visualizersCol']->find(array('status' => 1),array('_id' => 1)));
	    $agents_active = array_keys(array_merge($fa,$fu));

        // if common/anon user, list sampledata for active agents
        if ($_SESSION['User']['Type'] == 3 || $_SESSION['User']['Type'] == 2){
            $ft = $GLOBALS['sampleDataCol']->find(array(
                                                '$or' => array(
                                                    array("status" => $status, "agent"=> array('$not' => array('$exists' => 1)) ),
                                                    array("status" => $status, "agent"=> array('$in'  => $agents_active))
                                                )
                                            ),array('_id' => 1));

        // if admin user, list sampledata regardless agent status    
        }elseif ($_SESSION['User']['Type'] == 0){
            $ft = $GLOBALS['sampleDataCol']->find(array('status' => $status),array('_id' => 1));
        
        // if agent dev user, list sampledata for active agents + its own agents
        }elseif ($_SESSION['User']['Type'] == 1){
            $fr = $GLOBALS['agentsCol']->find(array('status' => 3,'_id'=> array('$in'=>$_SESSION['User']['AgentsDev'])),array('_id' => 1));
            $agents_owned = array_keys(iterator_to_array($fr));
            $ft = $GLOBALS['sampleDataCol']->find(array(
                                                '$or' => array(
                                                    array("status" => $status, "agent"=> array('$not' => array('$exists' => 1)) ),
                                                    array("status" => $status, "agent"=> array('$in'  => array_merge($agents_active,$agents_owned)))
                                                )
                                            ),array('_id' => 1));
        ...
        // list active sample data sets, regardless agent status
        $ft = $GLOBALS['sampleDataCol']->find(array('status' => $status),array('_id' => 1));
    }
