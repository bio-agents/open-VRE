...
function printFilePath_fromPath($path,$asRoot=0){
    ...
    $html = '
            ...
            <li class="agent-122 agent-list-item">
			<div class="col1">
              <div class="cont">
				   <div class="cont-col1">
...
function fromPrefix2Program($prefix){
        ...
        $agents   = $GLOBALS['agentsCol']->find(array('prefix' => array('$exists'=> true)));
	if (empty($agents)){
		$_SESSION['errorData']['Error'][]="Internal Error. Cannot extract any prefix from 'agents' collection";
		return 0;
	}
	foreach ( $agents as $agentId => $d ){
		if ($d['prefix'] == $prefix)
			return $d['title'];
	}
...
function prepMetadataResult($meta,$fnPath=0,$lastjob=Array() ){
        ...
        if (!isset($meta['agent']) && isset($lastjob['agent']))
                $meta['agent']=$lastjob['agent'];
        if (!isset($meta['agent']) && isset($lastjob['agentId']))
                $meta['agent']=$lastjob['agentId'];
	...
	if (!isset($meta['description']) ){
		$prefix = 0;
		if ($meta['agent']){
			$agent = $GLOBALS['agentsCol']->findOne(array('_id' => $meta['agent']));
			$prefix = $agent['prefix'];
		}
...
function validateMugFile($file,$is_output=false){
	...
	if (!isset($file['sources'])){
		if (isset($file['meta_data']['agent'])){
	         	$_SESSION['errorData']['Warning'][]="Invalid File. Attribute 'sources' required if metadata 'agent' is set";
			$val_score= 1;
			return array($val_score, $file);
		}else{
	...
	if ($is_output){
            if (!isset($file['meta_data']['agent'])){
		//TODO agent value is a valid agent_id
		$_SESSION['errorData']['Error'][]= "Invalid File. Attribute 'meta_data->agent' required if file is a agent output";
           	$val_score= 1;
		return array($val_score, $file);
	    }
