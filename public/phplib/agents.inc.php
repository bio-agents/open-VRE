...
function generateLogo($agentid) {
	$result = $GLOBALS['agentsDevMetaCol']->findOne(array("_id" => $agentid));

	$path = $GLOBALS['dataDir']."/".$result["user_id"]."/.dev/".$agentid."/logo/";
	if(!file_exists($path)) mkpath($path);

	$text = ($result["step3"]["agent_spec"]["name"]=="My Agent"? str_replace(array("_","-")," ",$agentid) : $result["step3"]["agent_spec"]["name"]);
