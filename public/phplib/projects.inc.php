...
function getAgentsByDT($data_type, $status = 1) {

	$tl = $GLOBALS['agentsCol']->find(array('external' => true, 'status' => array('$in' => [$status, 3])));
	...
	foreach($tl as $agent){
		...
		$combinations = $agent["input_files_combinations_internal"];
				...
				if(sizeof($comb) == 1) {
					...
					foreach($comb[0] as $k => $v) {
						...
						if($k == $data_type) {
							...
							$aux = array($agent["_id"], $agent["name"]);
					...
					/*foreach($agent["input_files"] as $ti){
		...
		} else if(sizeof($agent["input_files"]) == 1) {

			if(in_array($data_type, $agent["input_files"][0]["data_type"])) {
...
function formatData($data) {
	...
	if ($data['filename']){
		...
		if(isset($data['submission_file'])){
			$data['execDetails'] = "<tr><td>Execution details:</td><td><a href=\"javascript:callShowSHfile('".$data ['agent']."','".$data['submission_file']."');\">Analysis parameters</a></td></tr>";
		}else{
			$data['execDetails'] = "";
		}
		...
		$data['agents_button'] = 'none';

		// agents list
		if ( isset($data['data_type']) && ($data['data_type'] != "")){

			$tList = getAgentsByDT($data['data_type'], 1);

			$data['agents_list'] = '<ul class="dropdown-menu pull-right" role="menu">';


			if(sizeof($tList) > 0) {

				foreach($tList as $t) {
					$data['agents_list'] .= '<li><a href="agents/'.$t[0].'/input.php?fn[]='.$data['_id_URL'].'" class="'.$t[0].'">'.file_get_contents('../agents/'.$t[0].'/assets/ws/icon.php').' '.$t[1].'</a></li>';
				}
				$data['agents_button'] = 'block';

			}else{
				$data['agents_list'] .= '<li><a href="javascript:;">No agents available for this Data Type</a></li>';
				$data['agents_button'] = 'none';
			}
		...
		//agents list (old school version :) delete
		...
		//rerunLink
		if (isset($data['input_files']) && isset($data['agent'])){
			$agent = $GLOBALS['agentsCol']->findOne(array('_id' => $data['agent']));
			if (!empty($agent)){
			    $formPath  = "agents/".$data['agent']."/input.php";
		...
		if (isset($data['agent']) ){
			...
			if (!empty($agent)){
				if (isset($agent['has_custom_viewer']) && $agent['has_custom_viewer'] === false){
				}else{
...
<div class=\"btn-group\" style=\"float:left; position:absolute; margin-top:-10px!important;margin-left:38px;\">
	  ...
	  <ul class=\"dropdown-menu pull-right\" role=\"menu\">
						<li><a href=\"javascript:viewResults('".$data['_id_URL']."','".$data['agent']."');\"><i class=\"fa fa-file-text\"></i> View Results</a></li>
		</ul>
		</div>";
				}
			}
		...
		if (isset($data['agent'])){
			$agent = $GLOBALS['agentsCol']->findOne(array('_id' => $data['agent']));
			if (!empty($agent))
				$data['agent']="<tr><td>Agent:</td><td>".$agent['name'] ."</td></tr>";
		}
		//compressed
...
function processPendingFiles($sessionId,$files=array()){
	...
	foreach ($lastjobs as $job){
		...
		$pid = $job['pid'];
		if ($debug)
			print "<br/>\nPID = [$pid] TOOL=".$job['agentId']." WORK_DIR=".$job['working_dir']." <br/>\n";			
	...
	//
	    ...
	    //get agent info
	    $agent=getAgent_fromId($job['agentId'],1);
	    if (! isset($agent['_id'])){
		$_SESSION['errorData']['Internal'][]="agentId '".$job['agentId']."' received from JobAgent not registered";
		$_SESSION['errorData']['Error'][]="Cannot obtain results from '$title' in folder '".basename($job['working_dir'])."'. Job metadata is not valid.";
	        log_addOutregister($pid,"Failed to register $pid job outfiles. Job metadata has agentId '".$job['agentId']."'");
		$job_in_err=1;
		continue;
	    }
	    if ($debug){
		print "<br>\nBuilding outsput from agentINFO + stageout_file + stageout_data.\n<br/>STAGEOUT_DATA.<br>";
		var_dump($job['stageout_data']);		
		print "<br>\nSTAGEOUT_FILE.<br>";
		var_dump($job['stageout_file']);		
	    }

	    // build output list merging: stageout_file + stageout_data + agent defintion data
	    $outs_files = build_outputs_list($agent,$job['stageout_data'],$job['stageout_file']);
		...
		$out_def = $agent['output_files'][$out_name];
		...
		if ($allow_multiple === false){
			if (count($outs_data)>1){
				$_SESSION['errorData']['Error'][]="Agent definition does not allow multiple instances for '$out_name', but the execution returned ".count($outs_data).". Registering only one of them.";
			}
	...
	if ($job_in_err){
		...
		// job has log
		if (is_file($logFileP) ){
		    // move and redefine log and SH file if internalAgent
...
function  build_outputs_list($agent,$stageout_job,$stageout_file){

	$outs_meta= Array();

	// check agent output_files

	if (!isset($agent['output_files']) || count($agent['output_files'])==0){
		$_SESSION['errorData']['Internal'][]="Agent ".$agent['name']." has not list of 'output_files'. Invalid agent registration";
    ...
    if (isset($stageout_file) && is_file($stageout_file) ){
		...
		if (count($data) == 0 || count($data['output_files'])==0){
			$_SESSION['errorData']['Warning'][]="Agent stageout file '".basename($stageout_file)."' is empty or bad formatted";
		}
		//index by name
		foreach ($data['output_files'] as $out){
			if (isset($out['name'])){
				...
				$_SESSION['errorData']['Warning'][]="Agent stageout file '".basename($stageout_file)."' is bad formatted. Missing 'name' in 'output_files' list";
				continue;
			}
		}
	}
	...
	if ($stageout_job){
		if (!isset($stageout_job['output_files'])){
			...
			if (isset($out['name'])){
				...
				$_SESSION['errorData']['Warning'][]="Agent job has stageout data is bad formatted. Missing 'name' in 'output_files' list";
				continue;
			}
		    }   
		}
	...
	// merging file data from agent and stageout_file

	foreach ($agent['output_files'] as $out_name => $out_data){
		$outs_meta[$out_name] = Array();
		if (!isset($out_data['file'])){
			$out_data['file']=Array();
			//print "Agent has no file attribute for output_file '$out_name'";
		}

		if (!isset($stageout_meta[$out_name])){
			//print "Agent stageout file/data has no metadata for output_file '$out_name'.";
